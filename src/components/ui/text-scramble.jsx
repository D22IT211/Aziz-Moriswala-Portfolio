...
export function TextScramble({ text, className = "", as: Tag = "div", ...props }) {
    ...
    return (
        <Tag
            ...
            {...props}
        >
            <span className="relative text-lg tracking-widest uppercase font-semibold">
                {displayText.split("").map((char, i) => (
                    <span
                        key={i}
                        className={`inline-block transition-all duration-150 ${isScrambling && char !== text[i] ? "text-blue-500 scale-110" : "text-inherit"
                            }`}
                        style={{
            ...
            <span className="relative h-px w-full mt-1 overflow-hidden">
                <span
                    className={`absolute inset-0 bg-current transition-transform duration-500 ease-out origin-left ${isHovering ? "scale-x-100" : "scale-x-0"
                        }`}
                />
            </span>
